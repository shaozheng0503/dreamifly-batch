...
把一批提示词交给 [Dreamifly](https://dreamifly.com)，自动选模型、校验成本、逐条生成并下载 **图片 / 视频**。
...
**目录**：[它是什么](#它是什么) · [快速开始](#快速开始) · [成图示例](#成图示例) · [模型](#支持的模型) · [风格](#风格预设) · [用法参考](#用法参考) · [在 AI 里使用](#在-ai--agent-里使用) · [联动生态](#联动生态) · [排错与注意事项](#排错与注意事项)
...
- 一行一个 prompt 的 `prompts.txt`，或结构化的 `jobs.jsonl` / `.json` / `.yaml`
...
- 🎨 **8 模型 + 11 风格**：6 生图 + 2 视频，叠加卡通/动漫/油画/像素/乐高… 风格，一处切换
- 🎚️ **逐行内联参数**：`model= / style= / 16:9 / x2 / seed= / img= / secs= / res=`，每行独立覆盖
- 🧪 **零成本预检**：`--validate / --estimate / --dry-run / --check`，跑前看清每条、不调 API、不花积分
- 💸 **成本阻断**：单次 ≥5 积分的任务（视频 / 顶级模型）必须明确确认才跑
- ⬇️ **自动下载 + 边车 + `results.jsonl`**：产物落地、参数可复现、结果可被 Agent 读取
- ♻️ **缓存去重 + 断点续跑**：成功记录自动跳过，失败留在 `prompts.txt` 下次重试
- 🤖 **AI 原生**：自带 `SKILL.md` / `AGENTS.md`，Claude Code、Codex、自定义 Agent 都能直接调用

**适合**：批量跑几十条 prompt 并自动落地记录；逐行指定不同模型 / 比例 / 参考图；让 Agent 完成“写队列 → 校验 → 估算 → 预演 → 运行 → 汇报”；低成本探索后再升级到高质量模型；AI 漫剧 / 短剧批量分镜。

**不适合**：只想润色 prompt 不实际生成；想用 Midjourney / ComfyUI / fal.ai / Replicate 等其他平台；不希望本地保存文件；没确认预算就想直跑视频或高成本模型。
...
# 3) 四步预检（都不调用 API、不花积分）
python3 dreamify.py --validate    # 校验队列
python3 dreamify.py --estimate    # 估算成本
python3 dreamify.py --dry-run     # 解析预演
python3 dreamify.py --check       # 配置 / cookie / 连通性
...
结果在 `images/`，结构化记录写入 `results.jsonl`，成功移入 `done.txt`，失败留在 `prompts.txt` 下次自动续跑。

> 这条 **四步预检 → 运行 → 读结果** 是全仓库的统一流程，下文 CLI、AI 调用都复用它，不再重复展开。

> **需要登录态的模型**（`gpt-image-2`、`nano-banana-2` 和**所有视频模型**）要先配一次 Cookie；免登录模型（`Wai-SDXL-V150/V170`、`Z-Image-Turbo`、`Qwen-Image-Edit`）可跳过。鉴权 token 脚本会**自动计算**，你唯一要提供的就是 Cookie。

<details>
<summary>📎 配置登录态 Cookie（需登录模型才需要，点开看 5 步）</summary>

**第 1 步｜注册并登录**
打开 https://dreamifly.com ，用左下角的 GitHub / 微信 等方式登录。登录后左下角会显示你的用户名和**积分**。积分不足时生图/视频会返回 402。
...
> 备选：开发者工具 **Application / 存储 → Cookies → https://dreamifly.com** 也能看到各项；但 Network 里的 `Cookie` 请求头是已拼好的整行，直接复制最省事。

**第 4 步｜写入 `config/cookie.txt`**
```bash
cp config/cookie.txt.example config/cookie.txt
```
编辑 `config/cookie.txt`，把整行粘进去（**不要**带前面的 `Cookie:` 字样），保存。以 `#` 开头的行会被忽略，脚本取第一条非注释行作为 Cookie。
...
⚠️ Cookie 等同于你的登录凭证，已被 `.gitignore` 排除，**切勿提交或分享**。
...
<details>
<summary>边车 .json 与 results.jsonl 示例</summary>
...
> ⚠️ **成本阻断规则**：单次积分消耗 ≥5 的模型运行前必须确认；视频、`nano-banana-2` 未经明确同意不要直接跑。随时用 `python3 dreamify.py --list-models` 拉取在线最新列表。

<details>
<summary>平台模型页截图</summary>
...
|---|---|---|
| `model=...` | 选择模型（生图或视频） | 全部 |
| `style=...` | 风格预设（见[风格预设](#风格预设)） | 全部 |
| `16:9` | 宽高比，脚本会**自动换算匹配宽高**再发（这样比例才真正生效） | 全部 |
...
> **关于宽高比**：平台按 `width × height` 出图，只给 `aspectRatio` 而宽高仍是方形会出方图。脚本在你只给比例（如 `16:9`）时，会按所选模型的原生像素预算换算匹配宽高（如 gpt-image-2 → 1536×864）。平台预设比例：`16:9 21:9 4:3 3:2 5:4 1:1 4:5 2:3 3:4 9:16 9:21`；需要精确像素时用 `1024x768` 这种显式写法。

### 切换模型

三种方式指定模型，**优先级：内联参数 > 命令行 flag > config.json**。

```text
# 方式一：某一行临时指定（最灵活，可逐行不同模型）
...
```
方式三：改 `config/config.json` 的 `"model"` 字段（长期生效）。

> 模型名大小写不敏感。拿不准就先 `--list-models`，再 `--dry-run` 预演确认路由。
...
<details>
<summary>config/config.json 字段表</summary>
...
本仓库自带 `SKILL.md`（主 playbook）、`.cursor/` 和 `.claude/` 项目级 Skill、`AGENTS.md`（Codex / 通用 Agent 指令）。

对所有 Agent 流程一致：**让 AI 把提示词写进 `prompts.txt` / `jobs.jsonl` → 跑[四步预检](#快速开始) → `python3 dreamify.py` → 读 `--summary` / `results.jsonl` 汇报。** 视频、`nano-banana-2` 这类高成本任务会先停下来征求确认。

| 环境 | 安装 | 触发方式 |
|---|---|---|
| **Claude Code** | `./install.sh`（用户级，全局可用）或 `./install.sh .claude`（仅本项目） | 自然语言下达需求 |
| **Cursor** | 已内置 `.cursor/skills/`；全局装 `./install.sh cursor` | 打开仓库直接说 |
| **Codex** | 已内置 `AGENTS.md`，`cd` 进仓库启动 `codex` | 自然语言下达需求 |
| **自定义 Agent** | 把 `SKILL.md` / `AGENTS.md` 作为说明书 | 见下方工具定义 |
...
也可以不安装，直接 clone 后说「用这个目录里的 `dreamify.py` 批量生图」，Agent 会读 `SKILL.md` 按流程执行。

<details>
<summary>让你自己的 Agent 学会调用（工具定义示例）</summary>

本工具对 Agent 没有特殊要求——**任何能“读写文件 + 跑 shell 命令”的 Agent 都能用**：

1. **喂说明书**：把 `SKILL.md`（或 `AGENTS.md`）作为 system prompt / 工具说明。
2. **给它两个动作**：写文件（把提示词按内联语法追加到 `prompts.txt`）、跑命令（`python3 dreamify.py`，先四步预检）。
3. **让它读结果**：`--summary` 或解析 `results.jsonl`，把成功/失败和文件路径回报。
...
`dreamifly-batch` 可以作为其他 Skills 的**媒体生成执行层**：上游 Skill 负责理解内容、定风格、写 prompt 和 QA；本工具负责校验、估算、调用 Dreamifly、下载、记录和失败重试。

> **它负责“安全稳定地生成出来”，上游 Skill 负责“该生成什么”。** 任何需要生图/生视频的 Skill 都能复用这个模式。

下面这些图都由 `dreamifly-batch` 调用 Dreamifly 生成，展示“上游决定表达，本工具执行生成”的交接：

<table>
  <tr>
    <td width="50%" valign="top">
      <img src="./docs/skill-cases/case_xiaohei_gpt.png" alt="小黑正文配图" />
      <br /><strong>中文正文配图</strong><br />
      上游：<a href="https://github.com/helloianneo/ian-xiaohei-illustrations">ian-xiaohei-illustrations</a><br />
      读取 shot list / prompt，用 <code>gpt-image-2</code> 一次生成最终正文配图。
    </td>
    <td width="50%" valign="top">
      <img src="./docs/skill-cases/case-social-card.png" alt="社交图文卡片链路" />
      <br /><strong>社交图文卡片</strong><br />
      上游：<a href="https://github.com/op7418/guizang-social-card-skill">guizang-social-card-skill</a><br />
      缺素材时批量补齐封面图、章节图、隐喻图、背景图。
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <img src="./docs/skill-cases/case-slide-deck.png" alt="PPT 配图链路" />
      <br /><strong>PPT / Slide Deck 配图</strong><br />
      上游：<a href="https://github.com/jimliu/baoyu-design">baoyu-design</a><br />
      生成 PPT 页面所需配图，再由设计 Skill 插入 HTML/PPTX。
    </td>
    <td width="50%" valign="top">
      <img src="./docs/skill-cases/case-animation-video.png" alt="动画视频素材链路" />
      <br /><strong>动画视频素材</strong><br />
      上游：<a href="https://github.com/jimliu/baoyu-design">baoyu-design</a><br />
      生成场景图、角色图、背景图，再进入时间轴动画和 MP4 导出。
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <img src="./docs/skill-cases/ppt-generic/ppt-hero-ribbon.png" alt="网页 PPT 配图链路" />
      <br /><strong>网页 PPT 配图</strong><br />
      上游：<a href="https://github.com/op7418/guizang-ppt-skill">guizang-ppt-skill</a><br />
      按 S22(21:9) / S15(多图格) 槽位比例生成主视觉、章节图、隐喻图，填进单文件 HTML deck。
    </td>
    <td width="50%" valign="top">&nbsp;</td>
  </tr>
</table>

📖 **完整链路、模型实测结论、AI 漫剧分镜案例**见 [`docs/xiaohei-integration.md`](./docs/xiaohei-integration.md)（小黑配图详例）和 [`docs/skill-ecosystem.md`](./docs/skill-ecosystem.md)（跨 Skill 生态位）。

> 参考/对照项目：[`baoyu-image-gen`](https://github.com/JimLiu/baoyu-skills/tree/main/skills/baoyu-image-gen) 是通用多 provider 生图后端，可作为本工具的架构对照。
...
| 现象 | 原因 | 解决 |
|---|---|---|
| `HTTP 401` / `LOGIN_REQUIRED` | cookie 缺失或已过期 | 按[配置 Cookie](#快速开始)重新取一次写入 `config/cookie.txt` |
| `HTTP 402` / `INSUFFICIENT_POINTS` | 积分不足 | 充值，或改用免登录模型（`Wai-SDXL-V150` / `Z-Image-Turbo`） |
| `HTTP 400 Invalid steps` | 该模型对步数有硬性要求 | 脚本已按模型自动填；自定义用 `steps=20`/`30`（Wai）、`10`/`20`（Z-Image-Turbo） |
| 视频很慢 / `Connection reset` | 视频排队耗时长（可能 10 分钟以上） | 已默认 `video_timeout_seconds=1800`；失败项会保留，直接重跑 |
| 图生图/图生视频“没吃参考图” | 模型不支持 i2i 或忘了 `img=` | i2i 用 `Qwen-Image-Edit`/`gpt-image-2`/`nano-banana-2`；i2v 用 `Wan2.2-I2V-Lightning` 且必须 `img=` |
| 模型名报错 / 路由不对 | 名字拼错 | `--list-models` 查准确 id，再 `--dry-run` 预演确认 |
| 中文提示词效果差 | 模型不支持中文 | 用 `Z-Image-Turbo` / `Qwen-Image-Edit` / `gpt-image-2` / `nano-banana-2` |
| 参考图被拒 | 单图超过 10MB | 压缩到 10MB 以内；最多 9 张 |

**注意事项**

- ⚠️ **不要把 `config/cookie.txt` 提交到任何公开仓库**——它等同于你的登录凭证（已被 `.gitignore` 排除）。`results.jsonl` 可能含私密 prompt，同样不要公开。
- 💸 **视频很贵**：`Wan2.2-I2V-Lightning` 约 200 积分/次、`happyhorse-1.0` 约 150 起，且较慢；脚本对视频**不自动重试**以免重复扣费。
- 图生图（`img=`）参考图可为本地文件 / URL / `data:`URI，自动转无前缀 base64；单图 ≤10MB、最多 9 张、需登录。
- 积分不足（402）或登录失效（401）时脚本会立即停止并说明原因，已成功的不受影响；失败的留在 `prompts.txt` 直接重跑，也会写入 `failed.jsonl`，可用 `--retry-failed` 重新入队。

---

## 进阶

- **Skill 架构与鉴权原理**：见 [`docs/architecture.md`](./docs/architecture.md)。
- **零 API 单元测试**：`tests/` 覆盖解析、校验、估算。
